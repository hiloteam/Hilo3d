...
- `new Stage()` 与 `new Renderer()` 的同步构造默认选择 WebGL 2。
- `Stage.create()` / `Renderer.create()` 支持异步的 `auto`
  策略，默认先探测 WebGPU，不支持时选择 WebGL 2。
- 显式指定 `webgpu` 不会静默回退。
- `preserveDrawingBuffer`，以及 `alpha: true + premultipliedAlpha: false` 这类 WebGL
  2 专属组合，会让 `auto` 直接选择 WebGL 2。
- `Renderer` 构造函数直接返回选定的
...
![Hilo3d 渲染架构优势](./architecture-assets/hilo3d-rendering-advantages.png)
