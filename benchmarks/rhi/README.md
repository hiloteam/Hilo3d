...
[`manifest.json`](./manifest.json) fixes all ten scenarios from the RHI refactor plan, WebGL2 and
...
intervals. Paired legacy/RHI gates require at least seven rounds. A positive paired regression whose
confidence interval excludes zero fails independently of the Section 11.4 hard caps.
...
[`rhi-production.html`](../../test/performance/fixtures/rhi-production.html) loads the versioned
browser fixture protocol. Every scenario/backend/round launches the legacy and RHI renderer in
separate fresh pages, in deterministic seeded order. Both pages build the same manifest-owned scene
quality. The collector rejects reused page identities, changed quality, changed draw counts, and
different pixel hashes.
...
- Chromium CDP sampling at a one-byte interval profiles the independent 21-sample allocation pass,
  ...
  final five frames must remain within the temporary 16 KiB-per-frame hot-path TODO budget or the
  run aborts. Seven measured frames then execute immediately in the same sampler session. No
  quiescence frame is selected as a measurement, the sampler is never restarted between a window's
  proof and measurements, and all three retained profiles remain bounded. The classifier's dedicated
  renderer boundary excludes application-side scene mutation while retaining frame construction and
  public RHI lifecycle shells for legacy/RHI A/B comparison. The hot-path allocation counter
  includes SharedDrawPass/PreparedDraw execution and concrete context commands (including
  allocations in their helper/native descendants), while frame/pass/submission shell creation and
  WebGPU encoder creation/finalization are lifecycle costs outside that hot counter. In every marked
  Stage B frame, the end marker closes the synchronous renderer window before `waitForIdle`;
  sampling remains active while that wait settles, but the parser excludes work outside the marker
  pair. The wait completes before the next start marker, and the final wait completes before
  `stopSampling` returns the single profile that is split into frame samples;
...
`npm run test:rhi-benchmark-smoke` opens fresh legacy and RHI pages for a representative
...
and the measured vectors, then requires the RHI measured hot-path maximum to remain within the
temporary 16 KiB TODO budget, its renderer-allocation median not to exceed paired legacy, and
...
Temporary collector output belongs under the ignored `reports/rhi/` directory. Once collected, a
...
```sh
npm run benchmark:rhi:verify -- benchmarks/rhi/baselines/<rig-profile>/legacy.summary.json
```
...
npm run benchmark:rhi:gate -- \
  reports/rhi/candidate.raw.json.gz \
  benchmarks/rhi/baselines/<rig-profile>/legacy.summary.json \
  reports/rhi/candidate.gate.json \
  reports/rhi/candidate.gate.md
```

Paired statistical inference comes exclusively from the current raw artifact's legacy and RHI values
for the same scenario, backend, and round. Section 11.4 hard caps are separate cross-time
comparisons against the verified frozen legacy summary: each cap checks both current RHI and current
legacy drift against `frozen-legacy`. Cross-time rows never claim a paired confidence interval or
statistical significance, so a common regression in both current paths cannot disappear behind a
...
path `benchmarks/rhi/baselines/<rig-profile>/legacy.summary.json`; arbitrary temporary summaries and
symlinked summary/raw files are rejected. Output parents are resolved without writes and may not
...
```sh
npm run benchmark:rhi:collect -- reports/rhi/legacy.raw.json.gz
npm run benchmark:rhi:summarize -- reports/rhi/legacy.raw.json.gz reports/rhi/legacy.summary.json
npm run benchmark:rhi:freeze -- reports/rhi/legacy.summary.json reports/rhi/legacy.raw.json.gz
npm run benchmark:rhi:report -- reports/rhi/legacy.summary.json reports/rhi/report.md
```
