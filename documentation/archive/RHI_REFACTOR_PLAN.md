...
> **Archived design snapshot — 2026-09-20, source commit `6433334c`.** Historical proposals and
> validation records below are not current instructions or release evidence. Superseded by
> [RENDERING_ARCHITECTURE](../RENDERING_ARCHITECTURE.md) and the [current roadmap](../ROADMAP.md).
> Do not restore retired APIs, legacy renderers, or execute proposed commands from this snapshot.

> 状态：Phase 8 已完成（2026-07-16）目标版本：Hilo3d
> 2.x 核心约束：功能、画面和性能不得劣化；当前生产基线以
> [`RENDERING_ARCHITECTURE.md`](.././RENDERING_ARCHITECTURE.md) 为准
...
状态：已完成。生产树只保留 `SharedRendererDriver` 与
`src/render/rhi/backends/{webgl2,webgpu}`；性能回归改为当前 RHI 的跨提交冻结快照。
...
- [x] Renderer 和 Render Graph 不引用 WebGL/WebGPU native 类型。
- [x] RHI 是唯一硬件调用边界。
- [x] WebGL2 使用明确的 immediate command context，不伪装 deferred command buffer。
- [x] 两个 backend 的 pass/draw 顺序、结果和 frame 生命周期一致。
- [x] Device 与 Surface 解耦。
- [x] Shader 编译/variant 位于 RHI 之上。
- [x] WebGL pipeline/bind group/VAO 在非热路径预编译。
- [x] WebGL RHI context 没有软件 command replay；TODO：将临时 16 KiB/帧 hot-path 预算收紧回零分配。
- [x] WebGPU RHI concrete object 直接映射 native，无 Proxy 和重复 command list。
- [x] Render Frame 使用显式 context，不再依赖全局 semantic。
- [x] Render Graph 支持 imported/transient resource、依赖、裁剪、validation 和执行。
- [x] 主 pass、阴影、render target、后处理和 present 使用 shared pass。
- [x] 资源销毁考虑 in-flight submission 和 device generation。
- [x] WebGL2 和 WebGPU 功能与像素 parity 通过。
- [x] 旧 RHI wrapper、feature driver 与冲突测试已删除。
- [x] 10,000 帧资源 churn 后内存有界。
- [x] context/device loss 恢复门禁通过。
