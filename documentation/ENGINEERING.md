...
2，14 个页面执行 WebGPU，共 181 个 page/backend 组合。WebGPU-only 范围包含 Bloom、两个 Clustered
...
画廊对 96 个示例逐项维护标题、用途、主题与后端要求；缺失元数据、重复路径、已删除页面的残留条目都会令合同失败。默认展示 25 个精选；主题数量、搜索与后端筛选始终限定在当前 Highlights 或 All
