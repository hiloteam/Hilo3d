...
`quality` 接受 `low | medium | high | ultra`，并为未显式填写的 integration resolution、ray
step 和 screen-space visibility budget 提供固定默认值。`resolutionScale`、`stepCount`、`shadowSteps`
...
4. 根据 opaque scene depth 对视线做对数分布的 radiative-transfer integration；
5. previous view-projection reprojection、scene-depth rejection、3×3 neighborhood clamp 与 reactive
   history blend；
6. 以 `scene * transmittance + in-scattering` 在线性 HDR 合成；
7. 可选 TAA/TAAU；
8. ordinary transparent fallback、Bloom 与 display transform。
...
froxel XY 与 cluster tile 对齐，Z 使用 Camera near/far 的对数切片。物理纹理把 Z
slice 平铺进一个接近方形的 `rgba16float` 2D atlas；布局按最大 viewport、tile size 与 `zSlices`
计算并进入 `maxTextureDimension2D` 创建期要求。这样不扩大当前 Direct WGSL compute 的完整 2D
subresource ABI，也不会产生最高 `tileY × zSlices` 的狭长超限纹理。每个 texel 保存：
...
视线积分使用指数透射 `exp(-extinction × segmentLength)`，并用解析的 homogeneous segment
integral 累加 radiance，避免把雾当作简单的屏幕 alpha
blend。累计 radiance、density 和 transmittance 都有有限边界，零 extinction 分支不会产生除零或 NaN。
...
| Preset | integration scale | steps | visibility samples |
| ------ | ----------------- | ----- | ------------------ |
| low    | 0.25              | 12    | 2                  |
| medium | 0.375             | 18    | 3                  |
| high   | 0.5               | 28    | 5                  |
...
质量档只改变采样/积分预算，不改变 density、albedo、light units 或颜色空间。froxel
XY/Z 密度仍由 pipeline 的 `tileSize` 与 `zSlices` 决定，因此超大场景必须把 cluster index
budget、viewport limit 与体积质量作为一组预算，并通过 overflow
diagnostics 验证，而不是在 overflow 时随机丢灯。
...
- `examples/volumetric_neon_reliquary.html`：仓库内 Khronos Sponza、89 个 PBR bucket、8 个 animated
  spot light、12 个 floor light、7 个 local fog volume、16px tile、32 Z slice、TAAU、Bloom、camera
  tour 与 radiance/transmittance debug 的完整 Neon Reliquary 案例；
