...
- 用户已确认 Phase 8 收口：旧 WebGL2/WebGPU feature driver、旧 RHI wrapper 与 legacy
  factory 全部删除，生产树只保留 `SharedRendererDriver -> RenderGraphFrame -> RHI`。
- 唯一后端实现目录是 `src/render/rhi/backends/webgl2` 与
  `src/render/rhi/backends/webgpu`；共享层不得重新引入 backend feature stack。
- benchmark schema 已升级为 current-RHI-only：每轮只采集
  `rhi`，仍覆盖 WebGL2/WebGPU；未来候选与已冻结的当前实现快照做跨提交比较，不再在同一提交携带 legacy/RHI 双实现。
- 下方关于“正式 paired
  A/B 前不得删除 legacy”、`benchmark:rhi:gate`、旧路径和旧分支状态的内容仅是历史记录，已经被本节明确废止，不应作为后续执行指令。
- draw/command 热路径暂保留 **16 KiB/帧防爆上限**；继续消除真实分配，并在 profiler 稳定后收紧回 0。
