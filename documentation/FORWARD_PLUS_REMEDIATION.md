...
| FPC-06 | directional light 被复制到每个 cluster；AreaLight 被按点光计算；shadow light 静默丢阴影        | directional light 使用独立全局列表；AreaLight 使用与 Forward 相同的 LTC LUT，directional/spot/point shadow light 采样 renderer 共享 shadow atlas；资源或材质不兼容时才进入显式 Forward fallback | directional index、LTC 像素、shadow-atlas 与 fallback 路径测试 |
