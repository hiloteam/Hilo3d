...
交互门禁的桌面视口为 960 × 600（16:10），手机为 390 ×
844；场景对象、阴影和物理功能保持完整，高分辨率外观另由前面的审阅截图记录。采集前校验 Canvas 完整覆盖视口且已经发出原生绘制，再直接截取视口，避免动态页面逐元素滚动与连续帧稳定等待。像素区域避开界面，手机控件通过一次 DOM 采样逐项验证可见性及完整视口边界；物理动作、近景画面差异与最终 GPU 健康检查保持独立。
...
自动化通过 `?test=1`
使用 512px 阴影图与共享截图控制，普通页面仍使用原有 1024/2048px 规格。共享测试帧控制等待每次提交完成，再留出 50ms 的浏览器输入窗口；物理仍由真实 elapsed
time 推进，不在后台积压软件渲染帧。截图暂停期间的 fence 完成不会提前恢复 ticker，销毁后也不会重新唤醒。
`captureStableFrame()` 确认 native
draw 后暂停 ticker、等待 renderer 完成提交，再采集 compositor 像素，并在 `finally`
恢复；截图失败和 queue 失败均不能留下暂停的页面。Physics 独立 CI 工作组覆盖全部桌面与移动端用例，记录每例耗时与超时预算，避免与 CSM、Chromatic 堆积在同一按数量划分的分片。
