...
if (requestedBackend !== 'webgpu') {
    ...
    if (!sampleElement) throw new Error('Ray-tracing example UI is incomplete');
    ...
    const accumulation = runtime.engine.renderer.createStorageBuffer({
    ...
    runtime.engine.renderer.setRenderTarget(target, {
        present: true,
        takeOwnership: true,
        colorEncoding: 'srgb'
    });
    ...
    controller.attach(runtime.engine.canvas);
    ...
    async function stepAndRead(frames: number): Promise<RayTracingEvidence> {
        ...
        for (let frame = 0; frame < frames; frame += 1) {
            ...
            runtime.engine.frame(runtime.world, 1000 / 60);
        }
        await runtime.engine.renderer.waitForIdle();
        const readback = await target.readColorAttachment();
        return analyzeFrame(readback.data, runtime.engine.renderer.backend, frames);
    }
