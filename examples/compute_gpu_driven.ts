...
if (requestedBackend !== 'webgpu') {
    await startShowcase({ count: 48, floor: false });
} else {
    ...
    for (const mesh of [
        { x: -0.68, color: new Hilo3d.Color(0.92, 0.28, 0.18), metallic: 0 },
        { x: 0, color: new Hilo3d.Color(0.18, 0.86, 0.42), metallic: 0 },
        { x: 0.68, color: new Hilo3d.Color(0.18, 0.45, 0.96), metallic: 0.25 }
    ]) {
        createMeshEntity(runtime.world, {
            geometry: forwardGeometry,
            material: new Hilo3d.PBRMaterial({
                baseColor: mesh.color,
    ...
    const target = runtime.engine.renderer.createRenderTarget({
        width: TARGET_WIDTH,
        height: TARGET_HEIGHT,
        colorAttachments: [{ clearValue: { r: 0.015, g: 0.025, b: 0.05, a: 1 } }],
        depthStencilAttachment: { format: 'depth24plus', sampled: true }
    });
    const gaussianSplats = runtime.engine.renderer.createStorageBuffer({
    ...
    const forwardLights = runtime.engine.renderer.createStorageBuffer({
    ...
    const particles = runtime.engine.renderer.createStorageBuffer({
    ...
    runtime.engine.renderer.setRenderTarget(target, { present: true, takeOwnership: true });
    runtime.engine.frame(runtime.world, 1 / 60);
    await runtime.engine.renderer.waitForIdle();
    ...
    window.__HILO3D_COMPUTE_EFFECTS_RESULT__ = {
        ...
        gaussian,
        particle: { ...particle, simulatedParticles: PARTICLE_COUNT }
    };
