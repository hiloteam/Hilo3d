...
RHI v2 保留 WebGL immediate
execution 作为正式且明确的 backend 策略，不强制 WebGL 生成一份软件 command
...
RHI v2 使用 WebGPU-shaped API，原因是它天然区分：
...
WebGL2 是 RHI v2 的性能约束端。默认生产路径采用 immediate command context：Render
...
`RenderFrame` 是一次逻辑帧或一次显式离屏渲染事务，负责：
...
```ts
type RendererArchitecture = 'legacy' | 'rhi-v2';
```
...
### Phase 1：RHI v2 core 与 contract

工作：

- 在并行目录建立 RHI v2 types、validation 和 factory；
- 分离 device/surface；
...
- legacy/v2 不能在同一个 frame 同时修改同一 native context/device；
