vec4 textureEnvMap(sampler2D uTexture, vec3 position){
    return texture(uTexture, vec2(atan(position.x, position.z) * HILO_INVERSE_PI * 0.5+0.5,  acos(position.y) * HILO_INVERSE_PI));
}
...
vec4 textureEnvMapIncludeMipmapsLod(sampler2D uTexture, vec3 position, float lod){
    ...
    return texture(uTexture, vec2(uv.x / scale, (uv.y / scale / 2.0) + 1.0 - 1.0/pow(2.0, lod)));
}

#ifdef HILO_USE_SHADER_TEXTURE_LOD
    vec4 textureEnvMapLod(sampler2D uTexture, vec3 position, float lod){
        return textureLod(uTexture, vec2(atan(position.x, position.z) * HILO_INVERSE_PI * 0.5+0.5,  acos(position.y) * HILO_INVERSE_PI), lod);
    }

    vec4 textureEnvMapLod(samplerCube uTexture, vec3 position, float lod){
        return textureLod(uTexture, position, lod);
    }
#endif
