...
export class Std140Layout<Schema extends Std140Schema = Std140Schema> {
    ...
    /** Pack one field and return the exact byte interval dirtied by the write. */
            ...
            for (let column = 0; column < shape.columns; column++) {
                ...
                for (let row = 0; row < shape.rows; row++) {
                    ...
                    if (shape.scalar === 'bool') {
                        ...
                        view.setInt32(offset, Number(Boolean(item)), true);
                    } else {
                        ...
                        if (shape.scalar === 'float') view.setFloat32(offset, item, true);
                        else if (shape.scalar === 'uint') view.setUint32(offset, item, true);
                        else view.setInt32(offset, item, true);
                    }
                }
            }
        }
        return { byteOffset: field.offset, byteLength: field.byteLength };
    }
