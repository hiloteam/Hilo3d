...
import type { WebGPUV2CommandContext } from './WebGPUV2Commands';
import type { WebGPUV2Device } from './WebGPUV2Device';
import { WebGPUV2BindGroup, WebGPUV2GraphicsPipeline } from './WebGPUV2Pipeline';
import { WebGPUV2Buffer } from './WebGPUV2Resources';
...
export class WebGPUV2RenderPassStorage {
    readonly snapshot: RHIRenderPassDescriptorSnapshotStorage;
    readonly boundBindGroups: (WebGPUV2BindGroup | null)[];
    readonly boundVertexBuffers: (WebGPUV2Buffer | null)[];
    ...
    constructor(readonly owner: WebGPUV2Device) {
        ...
        this.boundBindGroups = new Array<WebGPUV2BindGroup | null>(
            owner.capabilities.limits.maxBindGroups
        ).fill(null);
        this.boundVertexBuffers = new Array<WebGPUV2Buffer | null>(
...
export class WebGPUV2RenderPass extends WebGPUV2Object implements RHIRenderPassEncoder {
    readonly contextId: number;
    readonly #storage: WebGPUV2RenderPassStorage;
    ...
    #pipeline: WebGPUV2GraphicsPipeline | null = null;
    #indexBuffer: WebGPUV2Buffer | null = null;
    ...
    constructor(
        readonly context: WebGPUV2CommandContext,
        nativePass: GPURenderPassEncoder,
        storage: WebGPUV2RenderPassStorage
    ) {
    ...
    setPipeline(pipeline: RHIGraphicsPipeline): void {
        ...
        if (!(pipeline instanceof WebGPUV2GraphicsPipeline) || pipeline.owner !== this.owner) {
            validationFailure('wrong-device', 'expected a WebGPU v2 pipeline', 'pipeline');
        }
    ...
    setBindGroup(index: number, bindGroup: RHIBindGroup, dynamicOffsets?: RHIUInt32View): void {
        ...
        if (!(bindGroup instanceof WebGPUV2BindGroup) || bindGroup.owner !== this.owner) {
            validationFailure('wrong-device', 'expected a WebGPU v2 bind group', 'bindGroup');
        }
    ...
    private validatePipelineCompatibility(pipeline: WebGPUV2GraphicsPipeline): void {
    ...
    private validateBindGroupLayout(
        index: number,
        bindGroup: WebGPUV2BindGroup,
        pipeline: WebGPUV2GraphicsPipeline
    ): void {
    ...
    private assertPipelineAndBindings(indexed: boolean): WebGPUV2GraphicsPipeline {
