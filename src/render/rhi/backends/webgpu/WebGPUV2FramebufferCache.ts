...
import { WebGPUV2TextureView } from './WebGPUV2Resources';

interface WebGPUV2FramebufferCacheRecord {
    readonly descriptor: Readonly<RHIRenderPassDescriptor>;
    readonly native: GPURenderPassDescriptor;
    lastUsed: number;
}

function usesView(record: WebGPUV2FramebufferCacheRecord, viewId: number): boolean {
...
function requireTextureView(
    ...
    if (!(value instanceof WebGPUV2TextureView) || value.owner !== device) {
        throw new RHIValidationError('wrong-device', 'expected a WebGPU v2 texture view', path);
    }
    return value;
}
...
export class WebGPUV2FramebufferCache {
    readonly #records: WebGPUV2FramebufferCacheRecord[] = [];
    #lastRecord: WebGPUV2FramebufferCacheRecord | null = null;
    #clock = 0;

    constructor(
        readonly device: WebGPUV2Device,
    ...
    lookup(descriptor: Readonly<RHIRenderPassDescriptor>): GPURenderPassDescriptor {
        ...
        const replacement: WebGPUV2FramebufferCacheRecord = {
