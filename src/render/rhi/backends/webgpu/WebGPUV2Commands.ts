...
import { WebGPUV2DestroyableObject, WebGPUV2Object } from './WebGPUV2Base';
import { nativeWebGPUOrigin } from './WebGPUV2Descriptors';
import type { WebGPUV2Device } from './WebGPUV2Device';
import { WebGPUV2Buffer, WebGPUV2Texture } from './WebGPUV2Resources';
import type { WebGPUV2Queue, WebGPUV2UploadAllocation } from './WebGPUV2Queue';
import { WebGPUV2RenderPass, type WebGPUV2RenderPassStorage } from './WebGPUV2RenderPass';

export function resetWebGPUV2FrameDiagnostics(target?: RHIFrameDiagnostics): RHIFrameDiagnostics {
    const diagnostics =
        target ??
        ({
            commandCount: 0,
...
export interface WebGPUV2FrameReferences {
    readonly objects: (WebGPUV2DestroyableObject | null)[];
    count: number;
}
...
function webGPUBuffer(device: WebGPUV2Device, buffer: RHIBuffer, path: string): WebGPUV2Buffer {
    device.assertUsable(buffer, path);
    if (!(buffer instanceof WebGPUV2Buffer) || buffer.owner !== device) {
        return validationFailure('wrong-device', 'expected a WebGPU v2 buffer', path);
    }
    return buffer;
}
...
function webGPUTexture(
    ...
    if (!(texture instanceof WebGPUV2Texture) || texture.owner !== device) {
        return validationFailure('wrong-device', 'expected a WebGPU v2 texture', path);
    }
    return texture;
}
...
export class WebGPUV2CommandContext extends WebGPUV2Object implements RHICommandContext {
    ...
    #activePass: WebGPUV2RenderPass | null = null;
    #externalImageUploadPhase = true;
    readonly #retainedReferences: WebGPUV2FrameReferences;
    readonly #uploadAllocation: WebGPUV2UploadAllocation = { buffer: null, offset: 0 };

    constructor(
        readonly queue: WebGPUV2Queue,
        nativeEncoder: GPUCommandEncoder,
        descriptor: RHIFrameDescriptor,
        retainedReferences: WebGPUV2FrameReferences
    ) {
        ...
        this.diagnostics = resetWebGPUV2FrameDiagnostics(descriptor.diagnostics);
    }
    ...
    beginRenderPass(descriptor: RHIRenderPassDescriptor): WebGPUV2RenderPass {
        this.assertOpen();
        ...
        const pass = new WebGPUV2RenderPass(this, nativePass, storage);
    ...
    retain(object: RHIDeviceOwnedObject): void {
        ...
        if (!(object instanceof WebGPUV2DestroyableObject) || object.owner !== this.owner) {
            validationFailure('wrong-device', 'expected a WebGPU v2 object', 'frame.resource');
        }
    ...
    /** @internal */
    closePass(pass: WebGPUV2RenderPass, storage: WebGPUV2RenderPassStorage): void {
    ...
    abortPass(pass: WebGPUV2RenderPass, storage: WebGPUV2RenderPassStorage): void {
    ...
    /** @internal */
    get retainedReferences(): WebGPUV2FrameReferences {
