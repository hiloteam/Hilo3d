...
function gpuScenePBRShader(
    ...
    const cacheKey = `${variant.key}|attributes=${withMaterialAttributes ? '1' : '0'}|gtao=${withGroundTruthAmbientOcclusion ? '1' : '0'}|cloud-shadow=${withCloudShadow ? '1' : '0'}`;
    ...
    if (withCloudShadow) {
        const binding = variant.textures.length * 2 + (withGroundTruthAmbientOcclusion ? 2 : 0);
        bindings.push(
            {
...
vec3 hiloEvaluateClusteredLight(
    ...
    return radiance * visibility * (lightDiffuse + lightSpecular);
}
void main() {
    ...
    private recordColorPasses(
        ...
        for (let index = 0; index < this.#physicalBuckets.length; index += 1) {
            ...
            parameters.configure(
                ...
                variant.textures.length + gtaoTextureCount + cloudShadowTextureCount
            );
            parameters.setBuffer(0, resources.frameBuffer);
            parameters.setBuffer(1, resources.objects);
            parameters.setBuffer(2, resources.visible);
            ...
            if (resources.cloudShadow !== null) {
                const cloudShadowIndex = variant.textures.length + gtaoTextureCount;
