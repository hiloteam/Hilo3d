...
export class Std140Layout<Schema extends Std140Schema = Std140Schema> {
    ...
    write<Name extends keyof Schema & string>(
        ...
        let dirtyEnd = 0;
        const markChanged = (offset: number): void => {
            dirtyStart = Math.min(dirtyStart, offset);
            dirtyEnd = Math.max(dirtyEnd, offset + 4);
        };
        ...
        return dirtyEnd > dirtyStart
            ? { byteOffset: dirtyStart, byteLength: dirtyEnd - dirtyStart }
            : { byteOffset: field.offset, byteLength: 0 };
    }
