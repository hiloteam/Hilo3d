...
export type WebGPUDriverOptions = Omit<RendererWebGPUOptions, 'backend' | 'preserveDrawingBuffer'>;

/** Adapter-only options used by the lightweight WebGPU support probe. */
export type WebGPUDriverSupportOptions = RendererSupportOptions;

/**
 * Observable WebGPU backend device lifecycle.
 *
...
function validateWebGPUAdapter(adapter: GPUAdapter, options: WebGPUDriverSupportOptions): void {
...
/**
 * Internal WebGPU backend using the same GLSL source of truth through the Naga compiler.
 *
 ...
 */
class WebGPUDriver extends RendererCore {
     ...
     */
    static async isSupported(options: WebGPUDriverSupportOptions = {}): Promise<boolean> {
        const snapshot: Required<WebGPUDriverSupportOptions> = Object.freeze({
    ...
    constructor(params: WebGPUDriverOptions = {}) {
        super();
    ...
    get gpuDevice(): GPUDevice {
        if (!this.device)
            throw new Error('WebGPU backend is not initialized; await renderer.ready');
        return this.device;
    }

    private get concreteDevice(): WebGPUDevice {
        const rhi = this.rhi;
        if (!rhi?.isReady) throw new Error('WebGPU backend RHI is unavailable');
        return rhi.device;
    }

    private assertInitializationActive(generation: number): void {
        if (this.destroyed || generation !== this.initializationGeneration) {
            throw new Error('WebGPU backend initialization was cancelled');
        }
    }
    ...
    private async initialize(generation: number): Promise<void> {
        ...
        try {
            ...
            const failure = cancelled
                ? new Error('WebGPU backend initialization was cancelled')
    ...
    /** Run a callback once initialization is complete, consistently across backends. */
    ...
    override async waitForIdle(): Promise<void> {
        await this.ready;
        if (this._recoveryState === 'recovering' && this._recoveryPromise) {
    ...
    override getExtension(name: string): object | null {
        return name === 'webgpu-native' ? this : null;
    }
    ...
    private renderInternal(stage: RendererScene, camera: Camera, fireEvent: boolean): void {
        ...
        if (this._recoveryState === 'failed' && this.recoveryError) {
            ...
            throw new Error(
                `Renderer cannot render because WebGPU ${context} failed: ${this.recoveryError.message}`,
    ...
    private getFrameCanvasTexture(context: GPUCanvasContext, ownsEncoder: boolean): GPUTexture {
        ...
        if (!surface) throw new Error('WebGPU backend surface is unavailable');
    ...
    private fireDeviceLifecycleEvent(type: WebGPUDeviceLifecycleEventName, detail: unknown): void {
        try {
            ...
            reportAsynchronousError(
                new Error(`WebGPU backend ${type} listener failed: ${cause.message}`, { cause })
            );
        }
    }
        ...
        void device.lost.then(
            info => {
                try {
                    ...
                    const cause = error instanceof Error ? error : new Error(String(error));
                    reportAsynchronousError(
                        new Error(`WebGPU backend device-loss observer failed: ${cause.message}`, {
                            cause
                        })
                    );
                }
    ...
    private async recoverDevice(lostDevice: GPUDevice, generation: number): Promise<void> {
        ...
        try {
            ...
            const rhi = this.rhi;
            if (!rhi) {
                throw new Error('WebGPU backend recovery configuration is unavailable');
            }
            await rhi.recover();
            this.assertInitializationActive(generation);
            await this.translator.initialize();
    ...
    private activateRecoveredDevice(
        ...
        const rhi = this.rhi;
        if (!rhi?.isReady) throw new Error('WebGPU backend recovery RHI is unavailable');
