...
import { setWebGLTextureCompareFunction } from './WebGLSamplerManager';
import {
...
import type WebGL2Driver from './WebGL2Driver';
...
class WebGLRenderTarget implements RenderTarget {
    ...
    private readonly owner: WebGL2Driver;
    ...
    /** Resolve a framebuffer only after its texture-allocation identities have been revalidated. */
    private requireCurrentFramebuffer(framebuffer: Framebuffer, label: string): WebGLFramebuffer {
        framebuffer.init();
        const nativeFramebuffer = framebuffer.framebuffer;
        if (!nativeFramebuffer) {
            throw new Error(`${label} framebuffer is unavailable`);
        }
    ...
    private configureSampledDepth(): void {
        ...
        setWebGLTextureCompareFunction(texture, compareFunction(depth.compare));
    }
            ...
            if (!this.destroyed && commit) {
                ...
                if (invalidated.length > 0) {
                    state.bindFramebuffer(
                        gl.FRAMEBUFFER,
                        this.requireCurrentFramebuffer(
                            this.drawFramebuffer,
                            'WebGL render-target draw'
                        )
                    );
    ...
    private resolveColorAttachments(): void {
        ...
        const sourceFramebuffer = this.requireCurrentFramebuffer(
            this.drawFramebuffer,
            'WebGL multisample source'
        );
        const destinationFramebuffers = this.resolveFramebuffers.map((framebuffer, index) =>
            this.requireCurrentFramebuffer(
                framebuffer,
                `WebGL color resolve destination ${String(index)}`
            )
        );
        const previousRead = state.currentReadFramebuffer;
        const previousDraw = state.currentDrawFramebuffer;
        state.bindFramebuffer(READ_FRAMEBUFFER, sourceFramebuffer);
        try {
            for (let index = 0; index < this.colorAttachmentCount; index++) {
                const destinationFramebuffer = destinationFramebuffers[index];
                if (!destinationFramebuffer) {
                    throw new Error(
                        `Missing WebGL2 resolve target for color attachment ${String(index)}`
                    );
                }
                state.bindFramebuffer(DRAW_FRAMEBUFFER, destinationFramebuffer);
    ...
    readColorAttachment(
        ...
        const sourceFramebuffer = this.requireCurrentFramebuffer(
            source,
            `WebGL color read source ${String(attachmentIndex)}`
        );
        const previousRead = state.currentReadFramebuffer;
        state.bindFramebuffer(READ_FRAMEBUFFER, sourceFramebuffer);
        try {
