...
import { createRenderer, isRendererBackendSupported } from './internal/RendererFactory';
import type {
 ...
 *
 * Use `Renderer.create()` so backend selection and initialization always complete before the
 * renderer is exposed. There is no render-call proxy or additional dispatch in the frame hot path.
 */
...
> implements RendererContract {
    ...
    private constructor() {
        throw new TypeError('Renderer cannot be constructed directly; use await Renderer.create()');
    }
