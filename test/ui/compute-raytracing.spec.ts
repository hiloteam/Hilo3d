...
test('keeps path tracing on the public compute and Render Graph APIs', () => {
    ...
    expect(source).toContain('fn waterHeight(');
    ...
    expect(source).toContain('fn projectToPreviousFrame(');
    expect(source).toContain('let spectralBand =');
    expect(source).toContain('let sunDisc =');
    expect(source).toContain('let destinationLayer =');
    expect(source).toContain('int latestRecord(');
    ...
    expect(source).toContain('accumulation[destinationRadianceIndex] =');
