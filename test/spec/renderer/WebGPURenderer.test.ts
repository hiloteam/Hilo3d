...
describe('WebGPURenderer initialization lifecycle', () => {
    ...
    it('sets dynamic stencil reference from the active draw target state', async () => {
        vi.spyOn(NagaShaderTranslator.prototype, 'initialize').mockResolvedValue(undefined);
        const fake = createFakeWebGPU();
        const renderer = new WebGPURenderer({ domElement: fake.canvas, stencil: false });
        await renderer.ready;
...
describe('WebGPURenderer optional vertex inputs', () => {
...
describe('WebGPURenderer primitive topology normalization', () => {
