...
function createShader(device: WebGPUV2Device, stage: 'vertex' | 'fragment') {
...
function createTriangleResources(device: WebGPUV2Device) {
...
describe('WebGPU RHI v2 native backend', () => {
    ...
    it('ignores upload remap callbacks that settle after loss or destroy', async () => {
        ...
        const lostDevice = new WebGPUV2Device(lossHarness.device);
        ...
        const destroyedDevice = new WebGPUV2Device(destroyHarness.device);
    ...
    it('reuses upload pages and row scratch across frames while native writes snapshot inputs', async () => {
        ...
        const device = new WebGPUV2Device(mock.device, diagnostics);
        const destination = device.createBuffer({
    ...
    it('uses globally unique device IDs and device-local object IDs', () => {
        ...
        const first = new WebGPUV2Device(firstHarness.device);
        const second = new WebGPUV2Device(secondHarness.device);
    ...
    it('normalizes descriptors and snapshots initial data before returning', () => {
        ...
        expect(buffer).toBeInstanceOf(WebGPUV2Buffer);
    ...
    it('keeps device construction headless and acquires a surface explicitly', () => {
        ...
        expect(texture).toBeInstanceOf(WebGPUV2Texture);
        expect(depth).toBeInstanceOf(WebGPUV2Texture);
