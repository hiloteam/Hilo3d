...
function createWebGPUPipeline(device: WebGPUV2Device) {
    const vertex = device.createShader({
        artifact: {
            backend: 'webgpu',
            stage: 'vertex',
...
describe('RHI v2 native object diagnostics', () => {
    ...
    it('settles WebGPU allocations on loss and remains balanced after replacement', async () => {
        ...
        const first = new WebGPUV2Device(firstMock.device, diagnostics);
        ...
        const replacement = new WebGPUV2Device(replacementMock.device, diagnostics);
