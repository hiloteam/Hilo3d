...
describe('RHI production collector', () => {
    ...
    it('runs seeded legacy/RHI pages independently and closes every session', async () => {
        ...
        expect(factory.requests.map(request => request.architecture)).toEqual([
            ...rhiBenchmarkPairedOrder(manifest.sampling.orderSeed, scenario.id, 'webgl2', 1),
            ...rhiBenchmarkPairedOrder(manifest.sampling.orderSeed, scenario.id, 'webgl2', 2)
        ]);
        expect(new Set(factory.sessions.map(session => session.metadata.isolationId)).size).toBe(4);
        const firstMetrics = raw.cases[0]?.rounds[0]?.results.legacy.metrics;
