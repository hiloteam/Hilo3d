...
class TimingProbe {
    ...
    measureRenderer(render: () => void): RHIBenchmarkTimingSample {
        ...
        const rhiExecuteCpuMs = this.#timings.queueEnd;
        const rhiCommandCpuMs = Math.max(0, this.#timings.graphExecute - this.#timings.queueEnd);
        return {
            frameBuildCpuMs,
            graphCompileCpuMs,
            rhiCommandCpuMs,
...
async function main(): Promise<void> {
    ...
    const architecture = queryParameter(query, 'architecture', ['rhi']) as RendererArchitecture;
