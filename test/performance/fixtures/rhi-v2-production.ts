...
import { WebGPUV2Queue } from '../../../src/render/rhi/backends/webgpu/WebGPUV2Queue';
...
} from './rhi-v2-postprocess-workload';
import {
...
class TimingProbe {
    ...
    resume(): void {
        ...
        this.#restores.push(
            ...
            wrapTimedMethod(RenderFrame.prototype, 'execute', add('renderFrame')),
            ...
            wrapTimedMethod(WebGPUV2Queue.prototype, 'endFrame', add('queueEnd')),
    ...
    measureRenderer(render: () => void): RHIBenchmarkTimingSample {
        ...
        const rhiExecuteCpuMs =
            this.architecture === 'rhi-v2' ? this.#timings.queueEnd : this.#timings.legacySubmit;
        const rhiCommandCpuMs =
            this.architecture === 'rhi-v2'
...
class BrowserBenchmarkFixture implements RHIBenchmarkProductionFixture {
    ...
    private nativeWebGPUDevice(): GPUDevice {
        ...
        const extension = this.#renderer.getExtension('rhi-v2') as RHIV2Extension | null;
        const device = extension?.device.nativeHandle;
        if (!device) fixtureFailure('RHI v2 WebGPU native device is unavailable');
        return device;
    }
...
if (
    ...
    document.documentElement.dataset['hiloRhiBenchmark'] === 'production-rhi-v2'
) {
