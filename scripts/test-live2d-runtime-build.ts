...
import { buildLive2DRuntime, runLive2DRuntimeCLI } from '../addon-live2d/tools/build-runtime.js';
...
void test('SDK builder preserves Core/licenses, audits modules and rejects native rendering', async (): Promise<void> => {
    ...
    try {
        ...
        const result = await buildLive2DRuntime({
            coreFile: sdk.core,
            frameworkDirectory: sdk.framework,
            outputDirectory: output
        });
        const copiedCore = result.files.find(path =>
            /live2dcubismcore\.[a-f\d]+\.min\.js$/.test(path)
        );
        ...
        assert.deepEqual(
            ...
            await readFile(new URL('../addon-live2d/LICENSE', import.meta.url))
        );
        await runLive2DRuntimeCLI([
            '--sdk',
            join(temporary, 'sdk'),
            '--output',
            join(temporary, 'shorthand')
        ]);
        assert.match(
            await readFile(join(temporary, 'shorthand/runtime.js'), 'utf8'),
            /createLive2DRuntime/
        );
        ...
        const firstProvider = await readFile(result.runtimeFile, 'utf8');
        const repeated = await buildLive2DRuntime({
            coreFile: sdk.core,
            frameworkDirectory: sdk.framework,
            outputDirectory: output
        });
        assert.equal(await readFile(repeated.runtimeFile, 'utf8'), firstProvider);
        await assert.rejects(
            buildLive2DRuntime({
                coreFile: sdk.core,
                frameworkDirectory: sdk.framework,
        ...
        assert.equal(
            await readFile(result.runtimeFile, 'utf8'),
            firstProvider,
            'A failed rebuild must not replace the active provider.'
        );
    } finally {
...
void test('deployed provider orders Core before Framework, forwards nonce, deduplicates and retries failed assets', async (): Promise<void> => {
...
import * as provider from '/runtime.js';
import * as otherProvider from '/other/runtime.js';
    ...
    try {
        ...
        const report = await page.evaluate(
            async (): Promise<{
                ...
                const provider = Reflect.get(globalThis, '__live2dProvider') as {
                    createLive2DRuntime(options?: {
                        nonce?: string;
                    }): Promise<{ apiVersion: number }>;
                };
                const first = provider.createLive2DRuntime({ nonce: 'sdk-test-nonce' });
                const second = provider.createLive2DRuntime();
                return {
        ...
        const coreRetry = await page.evaluate(
            async (): Promise<{ rejected: boolean; apiVersion: number }> => {
                const provider = Reflect.get(globalThis, '__live2dProvider') as {
                    createLive2DRuntime(): Promise<{ apiVersion: number }>;
                };
                let rejected = false;
                try {
                    await provider.createLive2DRuntime();
                } catch {
                    rejected = true;
                }
                return { rejected, apiVersion: (await provider.createLive2DRuntime()).apiVersion };
            }
        );
        ...
        const conflict = await page.evaluate(async (): Promise<string> => {
            const provider = Reflect.get(globalThis, '__otherLive2dProvider') as {
                createLive2DRuntime(): Promise<unknown>;
            };
            try {
                await provider.createLive2DRuntime();
                return '';
