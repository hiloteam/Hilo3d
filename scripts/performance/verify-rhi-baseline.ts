...
function parseSampling(value: unknown): RHIBenchmarkSamplingManifest {
    ...
    const parsed = {
        ...
        orderSeed: safeInteger(sampling['orderSeed'], 'manifest.sampling.orderSeed', 1),
...
function parseRound(
    ...
    if (parsed.orderPosition !== 0 && parsed.orderPosition !== 1) {
        failure(`${context}.orderPosition must be 0 or 1`);
    }
    if (
        parsed.orderPosition !==
        rhiBenchmarkPairedOrder(sampling.orderSeed, scenarioId, backend, roundNumber).indexOf(
            'legacy'
        )
    ) {
        failure(`${context}.orderPosition differs from the deterministic seeded order`);
    }
    return parsed;
}
