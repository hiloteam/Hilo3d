...
export interface RHIProductionFixtureSmokeOptions {
    ...
    /** NON-EVIDENCE diagnostic selector; the default remains the strict paired matrix. */
...
async function observeFixture(
    ...
    const query = new URLSearchParams({
        ...
        round: '1',
        orderPosition: architecture === 'legacy' ? '0' : '1',
    ...
    try {
        ...
        if (
            result.metadata.scenarioId !== scenario.id ||
...
export async function runRHIProductionFixtureSmoke(
    ...
    if (architectures.length < 1 || architectures.length > ARCHITECTURES.length) {
        smokeFailure('architecture selector must contain one diagnostic or the paired matrix');
    }
        ...
        for (const scenario of scenarios) {
            for (const backend of backends) {
                ...
                if (observations.length === 1) {
                    ...
                    if (!observation) smokeFailure('diagnostic smoke observation is incomplete');
                ...
                if (!legacy || !candidate) smokeFailure('paired smoke observation is incomplete');
                const hotVector = candidate.allocationSamples
                    .map(sample => String(sample.rhiHotPathBytes))
                    .join(',');
                const candidateRendererVector = candidate.allocationSamples
                    .map(sample => String(sample.rendererBytes))
                    .join(',');
                const legacyRendererVector = legacy.allocationSamples
                    .map(sample => String(sample.rendererBytes))
                    .join(',');
                const candidateQuiescenceMatrix = candidate.allocationQuiescenceWindows
                    .map(
                        windowSamples =>
                            `[${windowSamples
                                .map(sample => String(sample.rhiHotPathBytes))
                    ...
                    .join(',');
                const legacyQuiescenceMatrix = legacy.allocationQuiescenceWindows
                    .map(
                        windowSamples =>
                            `[${windowSamples
                                .map(sample => String(sample.rhiHotPathBytes))
                ...
                if (
                    ...
                    const samples = candidate.allocationSamples
                        .map(sample => String(sample.rhiHotPathBytes))
                        .join(', ');
                    const detail = candidate.allocationHotFrames
                        .map(frame => `${String(frame.bytes)} ${frame.frame}`)
                        .join('\n');
                    smokeFailure(
                ...
                if (candidate.allocation.rendererBytes > legacy.allocation.rendererBytes) {
                    const candidateSamples = candidate.allocationSamples
                        .map(sample => String(sample.rendererBytes))
                        .join(', ');
                    const legacySamples = legacy.allocationSamples
                        .map(sample => String(sample.rendererBytes))
                        .join(', ');
                    const candidateDetail = candidate.allocationRendererFrames
                        .map(frame => `${String(frame.bytes)} ${frame.frame}`)
                        .join('\n');
                    const legacyDetail = legacy.allocationRendererFrames
                        .map(frame => `${String(frame.bytes)} ${frame.frame}`)
                        .join('\n');
                    smokeFailure(
