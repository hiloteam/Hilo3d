    ...
    for (let caseIndex = 0; caseIndex < raw['cases'].length; caseIndex += 1) {
        ...
        for (let roundIndex = 0; roundIndex < benchmarkCase['rounds'].length; roundIndex += 1) {
            ...
            if (
                ...
                rawFailure(`${roundContext}.order must contain only rhi`);
            }
            const results = record(round['results'], `${roundContext}.results`);
            exactKeys(results, ['rhi'], `${roundContext}.results`);
            for (const architecture of ['rhi'] as const) {
                ...
                if (observedDrawCount !== scenario.quality.drawCount) {
                ...
                if (pixelHash !== expectedPixelHash) {
                    rawFailure(`${context} pixel hashes differ across rounds`);
                }
...
export function summarizeRHIRawBenchmarkCapture(
    ...
    return {
        ...
        cases: raw.cases.map(benchmarkCase => {
            const firstResult = benchmarkCase.rounds[0]?.results.rhi;
            if (!firstResult) throw new Error('raw case is missing its first RHI round');
            return {
                ...
                rounds: benchmarkCase.rounds.map(round => {
                    const result = round.results.rhi;
