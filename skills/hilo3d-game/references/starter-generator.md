...
Generated starters intentionally depend only on `hilo3d`. Add `@hilo/addon-particle` or
`@hilo/addon-physics` and the selected Rapier peer only when the requested game actually uses those
systems; never make either addon a default starter dependency.
